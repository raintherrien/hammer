...
struct world_config_appstate {
	...
	char scale_edit_buf[WORLDCONF_NUM_EDIT_BUFFER_LEN];
...
world_config_entry(DL_TASK_ARGS)
{
	...
	snprintf(world_config->scale_edit_buf, WORLDCONF_NUM_EDIT_BUFFER_LEN,
...
world_config_gl_frame(void *world_config_)
{
	...
	const char *scale_label = "World Scale:";
	const char *seed_label = "Seed:";
	const char *scale_err_num_label = "Must be a number";
	const char *scale_err_range_label = "Must be between 1 and 5";
	...
	struct text_opts scale_label_opts = {
		TEXT_OPTS_DEFAULTS,
		.xoffset = padding,
	...
	struct edit_opts scale_edit_opts = {
		EDIT_OPTS_DEFAULTS,
		.xoffset = padding + 128,
		.yoffset = padding + font_size,
	...
	struct text_opts scale_err_opts = {
		TEXT_OPTS_DEFAULTS,
		.color   = 0xff0000ff,
	...
	gui_text(scale_label, strlen(scale_label), scale_label_opts);
	gui_text(seed_label, strlen(seed_label), seed_label_opts);
	gui_edit(world_config->scale_edit_buf, WORLDCONF_NUM_EDIT_BUFFER_LEN, scale_edit_opts);
	...
	unsigned long long parsed_seed;
	{
		...
		size_t buflen;
		errno = 0;
		buflen = strlen(world_config->scale_edit_buf);
	...
	if (parsed_scale == ULONG_MAX) {
		gui_text(scale_err_num_label, strlen(scale_err_num_label), scale_err_opts);
	} else if (parsed_scale < 1 || parsed_scale > 5) {
		gui_text(scale_err_range_label, strlen(scale_err_range_label), scale_err_opts);
		parsed_scale = ULONG_MAX;
	}
